...
export const revokeCompanyAccess = async (req, res) => {
  try {
    ...
    // Revoke selected company
    access.isActive = false;
    access.revokedAt = new Date();

    // Find another active company access
    const remainingAccess = user.companyAccess.find(
      (c) =>
    ...
    if (remainingAccess) {
      // Switch user to another active company
      user.role = remainingAccess.role;
      user.companyId = remainingAccess.companyId;
    } else {
      // No active companies left
      user.role = "user";
      user.companyId = null;
    }
